...
from apps.api.errors import ProblemDetailException, ProblemDetail, ValidationError, DetailType
...
class GeneralResponse(HttpResponse):
    def __init__(self, request, data: Optional[ResponseType] = None, **kwargs):
        ...
        if data is not None:
            ...
            skip_content_type_check = settings.DEBUG and request.GET.get("debug") is not None

            if skip_content_type_check or any(x in ["*/*", "application/json"] for x in content_types):
